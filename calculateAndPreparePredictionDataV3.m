function [ outputModel, outputModelInterp, outputParams, LCIStopCrit] = calculateAndPreparePredictionDataV2( parameters, table, gas, predictionStructs, validData )
    ...
    frc                   = gas.General.frcao*1000;
    cev                   = gas.General.cev_ds*1000;
    cet                   = gas.General.cet_norm;                      
    ...
    for i = 1:size(predictionStructs, 1)
        ...
        for j = 1:length(predictionStructs{1,1})
            ...
            TO2_5      = findZeroPassingPoint(fCut);
...
%             end
            ...
            FRCextrapolated = 10^coeff(2)*TO2_5.^(-coeff(1));
            FRC = FRCextrapolated;
            %FRC         = frc(breath0025);
            
            CEV         = TO2_5*FRC;
            LCIrelError = (TO2_5-lci0025)/lci0025;
            
            cev_measured_to_predicted_end = linspace(cev(breath0025), CEV);
            fCet = @(x)fHandleCC(fParamsCC,x);
            cetM = [cet(1:breath0025)'                     fCet(cev_measured_to_predicted_end)];
            lciM = [cev(1:breath0025)'./frc(1:breath0025)' cev_measured_to_predicted_end./frc(breath0025)'];
            
            [ m0, m1, m2 ] = getMomentRatios( cetM, lciM );

            saveRegion  = 1+length(generalInfo)+(j-1)*lengthHeader:1+length(generalInfo)+j*lengthHeader-1;
            outputModel(i+2, saveRegion) = num2cell([TO2_5, LCIrelError, CEV, FRC, m0, m1, m2, r2CC]);
        end
    end
    ...
    LCIStopCrit = getLCIFromPredictionStopCriteria(outputModel, offsetGeneralInfo, lengthHeader, gas, predictionStructs);
