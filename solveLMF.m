function [xf, S, cnt] = solveLMF(varargin)
% solveLMF  Solve a Set of Nonlinear Equations in Least-Squares Sense.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
% [Xf, Ssq, CNT] = solveLMF(FUN,Xo,Options)
...
%         Options = solveLMF('default'), or by a set of Name/Value pairs:
%         Options = solveLMF('Name',Value, ... ), or updating the Options
%                   set by calling
%         Options = solveLMF(Options,'Name',Value, ...).
%
...
%   [x,ssq,cnt]=solveLMF(ros,[-1.2,1],'Display',1,'MaxIter',50)
% returns   x = [0.4556; 0.2059],  ssq = 0.2966,  cnt = 18.
%
% Note:   Users with old MATLAB versions (<7), which have no anonymous
% functions implemented, should call solveLMF with named function for
% residuals. For above example it is
%   [x,ssq,cnt]=solveLMF('rosen',[-1.2,1]);
...
%                       modified its calling from inside solveLMF.
...
%               Updating Options
elseif isstruct(varargin{1}) % Options=solveLMF(Options,'Name','Value',...)
    if ~isfield(varargin{1},'Display')
        error('Options Structure not correct for solveLMF.')
    end
...
%               Pairs of Options     
elseif ischar(varargin{1})   % check for Options=solveLMF('Name',Value,...)
   Pnames=char('display','funtol','xtol','maxiter','scaled');
   if strncmpi(varargin{1},Pnames,length(varargin{1}))
      xf=solveLMF('default'); % get default values
      xf=solveLMF(xf,varargin{:});
      return
   end
end

%   solveLMF(FUN,Xo,Options)
    %%%%%%%%%%%%%%%%%%%%%%%%
...
if nargin>2                 %   OPTIONS
    if isstruct(varargin{3})
        ...
        for i=3:2:size(varargin,2)-1
            options=solveLMF(options, varargin{i},varargin{i+1});
        end
    end
else
    if ~exist('options','var')
